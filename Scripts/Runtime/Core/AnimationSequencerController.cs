...
namespace BrunoMikoski.AnimationSequencer
{
    ...
    public class AnimationSequencerController : MonoBehaviour
    {
        ...
        public AnimationStepBase GetStepAtIndex(int index)
        {
            return animationSteps[index];
        }

        public void DuplicateAtIndex(int index)
        {
            System.Collections.Generic.List<AnimationStepBase> tempList = new System.Collections.Generic.List<AnimationStepBase>(animationSteps);
            AnimationStepBase copy = Sirenix.Serialization.SerializationUtility.CreateCopy(tempList[index]) as AnimationStepBase;
            if (copy.GetType() == typeof(DOTweenAnimationStep))
            {
                (copy as DOTweenAnimationStep).ResetCache();
            }
            tempList.Add(copy);
            animationSteps = tempList.ToArray();

            //UnityEditor.EditorUtility.SetDirty(this);
        }

        public void PasteToObject(AnimationStepBase source, int indexTarget)
        {
            if (indexTarget != -1)
            {
                AnimationStepBase copy = Sirenix.Serialization.SerializationUtility.CreateCopy(source) as AnimationStepBase;

                if (copy.GetType() == typeof(DOTweenAnimationStep))
                {
                    (copy as DOTweenAnimationStep).ResetCache();
                }
                animationSteps[indexTarget] = copy;
            }
        ...
        public void ResetCacheAtStepIndex(int index)
        {
            if (animationSteps[index].GetType() == typeof(DOTweenAnimationStep))
            {
                (animationSteps[index] as DOTweenAnimationStep).ResetCache();
            }
        ...
        [ContextMenu("ResetCacheAllSteps")]
        void ResetCacheAllSteps()
        {
            for (int i=0;i< animationSteps.Length;i++)
            {
                if (animationSteps[i].GetType() == typeof(DOTweenAnimationStep))
                {
                    (animationSteps[i] as DOTweenAnimationStep).ResetCache();
                }
            }
        ...
        public void TriggerOnStartEvent()
        {
            onStartEvent?.Invoke();
        }
