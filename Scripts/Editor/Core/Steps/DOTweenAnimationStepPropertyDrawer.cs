...
namespace BrunoMikoski.AnimationSequencer
{
    ...
    public class DOTweenAnimationStepPropertyDrawer : AnimationStepBasePropertyDrawer
    {
        ...
        private void AddNewActionOfType(SerializedProperty actionsSerializedProperty, Type targetType)
        {
            ...
            //if (arrayElement.managedReferenceValue is DOTweenActionBase)
            //{
            //    (arrayElement.managedReferenceValue as DOTweenActionBase).ResetCache(targetGameObject);
            //}    
        ...
        {
            var isDurationNormalExclude = property.FindPropertyRelative(DOTweenAnimationStep.IS_USE_SCRIPTABLE_DURATION_NAME);
            string durationTypeExclude = string.Empty;
            if (isDurationNormalExclude.boolValue)
            {
                durationTypeExclude = DOTweenAnimationStep.DURATION_NAME;
            }
            else
            {
                durationTypeExclude = DOTweenAnimationStep.SCRIPTABLE_DURATION_NAME;
            }

            DrawBaseGUI(position, property, label, "actions", "loopCount", "loopType", DOTweenAnimationStep.IS_USE_SCRIPTABLE_DURATION_NAME,
                DOTweenAnimationStep.DURATION_NAME, DOTweenAnimationStep.SCRIPTABLE_DURATION_NAME);
            ...
            if (property.isExpanded)
            {
                ...
                EditorGUI.LabelField(position, DOTweenAnimationStep.DURATION_NAME.ToUpper(), EditorStyles.boldLabel);

                position.y += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing;

                SerializedProperty isUseScriptableDurationProperety = property.FindPropertyRelative(DOTweenAnimationStep.IS_USE_SCRIPTABLE_DURATION_NAME);
                EditorGUI.PropertyField(position, isUseScriptableDurationProperety);

                position.y += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing;

                if (isUseScriptableDurationProperety.boolValue)
                {
                    SerializedProperty tmpProperty = property.FindPropertyRelative(DOTweenAnimationStep.SCRIPTABLE_DURATION_NAME);
                ...
                {
                    SerializedProperty tmpProperty = property.FindPropertyRelative(DOTweenAnimationStep.DURATION_NAME);
                    EditorGUI.PropertyField(position, tmpProperty);
