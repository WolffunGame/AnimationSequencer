...
namespace BrunoMikoski.AnimationSequencer
{
    ...
    public class AnimationSequencerControllerCustomEditor : Editor
    {
        ...
        private void ResetCacheAtIndex(int index)
        {
            sequencerController.ResetCacheAtStepIndex(index);
        }

        private void DuplicateItem(int index)
        {
            sequencerController.DuplicateAtIndex(index);
            ...
            if (rect1.Contains(current.mousePosition) && current.type == EventType.ContextClick)
            {
                ...
                menu.AddItem(
                    ...
                    () =>
                    {
                        ...
                        ContextClickUtils.SetSource(sequencerController.GetStepAtIndex(index));
                    }
                );
                if (ContextClickUtils.CanPasteToTarget(element))
                {
                    menu.AddItem(
                        ...
                        () =>
                        {
                            ...
                            Undo.RecordObject(target, "Paste sequence step");

                            ContextClickUtils.ApplySourceToTargetByCustom(index, sequencerController.PasteToObject);
                            reorderableList.serializedProperty.serializedObject.ApplyModifiedProperties();
                ...
                menu.AddItem(
                    ...
                    () =>
                    {
                        Undo.RecordObject(target, "Duplicate sequence step");
                ...
                menu.AddItem(
                    new GUIContent("Reset Item"),
                    false,
                    () =>
                    {
                        Undo.RecordObject(target, "Reset cache sequence step at index: " + index);
